...
public class CartItemOptionController {
    ...
    public ResponseEntity<CartItemOption> createCartItemOption(@Valid @RequestBody CartItemOption cartItemOption)
            throws IdInvalidException {
        CartItemOption createdCartItemOption = cartItemOptionService.createCartItemOption(cartItemOption);
        return ResponseEntity.ok(createdCartItemOption);
    }

    @PutMapping("/cart-item-options")
    @ApiMessage("Update cart item option")
    public ResponseEntity<CartItemOption> updateCartItemOption(@RequestBody CartItemOption cartItemOption)
            throws IdInvalidException {
        CartItemOption updatedCartItemOption = cartItemOptionService.updateCartItemOption(cartItemOption);
    ...
    public ResponseEntity<ResultPaginationDTO> getAllCartItemOptions(
        ...
        ResultPaginationDTO result = cartItemOptionService.getAllCartItemOptions(spec, pageable);
        return ResponseEntity.ok(result);
    }

    @GetMapping("/cart-item-options/{id}")
    @ApiMessage("Get cart item option by id")
    public ResponseEntity<CartItemOption> getCartItemOptionById(@PathVariable("id") Long id) throws IdInvalidException {
    ...
    @DeleteMapping("/cart-item-options/cart-item/{cartItemId}")
    @ApiMessage("Delete all cart item options by cart item id")
    public ResponseEntity<Void> deleteCartItemOptionsByCartItemId(@PathVariable("cartItemId") Long cartItemId) {
        cartItemOptionService.deleteCartItemOptionsByCartItemId(cartItemId);
        return ResponseEntity.ok().body(null);
    }
}
